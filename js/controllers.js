...
app.controller("mainCtrl", function(
  ...
  $http
    .get("json/pools.json") // Get pools.json API (corrected file used)
  ...
  $scope.getPool = function(poolId) {
    ...
    $http
      ...
      .then(function(response) {
        ...
        $scope.stake.values = [2, 1, 0.5, 0.2];
        prepSel($scope.pool);
    ...
    function prepSel(pool) {
      //Prepare selections in pool to receive "leg.id---selection.id" info for ng-model
  ...
  $scope.getSelections = function(leg, selection) {
    //Get selected selections
    console.log(
      "Leg ID=" + leg.id + " and Selection ID=" + selection.id + ": ",
      selection
    );
  };
  ...
  $scope.numberOfLines = function() {
    ...
    data.numberOfLines = calcNumberOfLines(
      data.selections.summary,
      data.selections.length
    );
    return data.numberOfLines;

    function getNumberOfSelectionsPerLeg(arr) {
      //Get number of selections per leg
      var legs = [];
      arr.map(function(e, index) {
        //Get all legs together
        legs.push(e.split("---")[0]);
      });

      var summary = legs.reduce(function(prev, item) {
        //Get the number of leg occurences
        if (item in prev) {
          prev[item]++;
        } else {
          prev[item] = 1;
    ...
    function calcNumberOfLines(obj, totalSelections) {
      //Calculate number of lines
      var numOfLines;
      var i = 0;
      Object.keys(obj).forEach(function(key, index) {
        //looping through unique legs
        if (obj[key] >= 2) {
          i += 1;
        }
      });
  ...
  $scope.placeBet = function() {
    ...
    if ($scope.dataOut.selections.length > 0) {
      console.log(
        "You have selected: " +
          $scope.dataOut.selections +
          " in the pool: " +
          $scope.dataOut.poolId,
        $scope.dataOut
      );
